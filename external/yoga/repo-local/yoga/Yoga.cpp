...
static void EmptyNodeLayoutEventCallback(YGNodeConstRef node, int32_t layoutType) {}
static YGNodeLayoutEventCallback sNodeLayoutCallback = &EmptyNodeLayoutEventCallback;
...
bool YGLayoutNodeInternal(
  ...
  if (layoutType == LayoutType::kLayout || layoutType == LayoutType::kCachedLayout) {
    sNodeLayoutCallback(node, (int32_t) layoutType);
  }
#endif
...
YOGA_EXPORT void YGRegisterNodeLayoutEvent(YGNodeLayoutEventCallback callback) {
  sNodeLayoutCallback = callback ? callback : &EmptyNodeLayoutEventCallback;
}
